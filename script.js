document.addEventListener('DOMContentLoaded', () => {
    ...
    console.log("Script.js vFINAL_CORREGIDO: DOMContentLoaded.");
    ...
    async function cargarDatos() {
        ...
        try {
            ...
            if (!responseTasas.ok) throw new Error(`Error cargando tasas_bancos.json (${responseTasas.status})`);
            tasasBancosData = await responseTasas.json();
            
            const responseFci = await fetch('fci_data.json?v=' + versionCache, fetchConfig);
            if (!responseFci.ok) throw new Error(`Error cargando fci_data.json (${responseFci.status})`);
    ...
    // ***** FUNCIÓN renderizarGraficoBanco AÑADIDA DE NUEVO *****
    ...
    // ***** FUNCIÓN actualizarGraficoGeneralYResumen AÑADIDA DE NUEVO *****
