document.addEventListener('DOMContentLoaded', () => {
    ...
    console.log("Script.js vNUEVA_ESTRATEGIA_INNERHTML: DOMContentLoaded.");
    ...
    function calcularYMostrarResultados() {
        console.log("[CM] Calcular y mostrar resultados INICIADO...");
        // ... (verificaciones iniciales y de datos como antes) ...
        ...
        // ... (resto de la lógica de cálculo de FCI como antes) ...
        ...
        console.log("[CM] Calcular y mostrar resultados FINALIZADO.");
    }

    // ***** NUEVA VERSIÓN DE generarSimulacionPorBanco USANDO innerHTML *****
    function generarSimulacionPorBanco(banco, montoInicial, bancoIndex) {
        console.log(`[GSB innerHTML] Iniciando para: ${banco.banco}`);
        if (!simulacionDetalladaPfDiv) {
            console.error("[GSB innerHTML] FATAL: simulacionDetalladaPfDiv es null.");
            return;
        }
        ...
        // 1. Construir las filas de la tabla como un string HTML
        let filasTablaHTML = '';
        ...
        // 2. Construir el HTML completo para el contenedor del banco
        ...
        // 3. Añadir este bloque de HTML al div principal de una sola vez
        try {
            // Usamos 'insertAdjacentHTML' que a veces es más seguro que 'innerHTML +='
            simulacionDetalladaPfDiv.insertAdjacentHTML('beforeend', bancoHTML);
            console.log(`[GSB innerHTML] Bloque HTML para ${banco.banco} añadido al DOM.`);
        } catch(e) {
            console.error(`[GSB innerHTML] ERROR al insertar el bloque HTML para ${banco.banco}:`, e);
            return;
        }
        
        // 4. Renderizar el gráfico AHORA que el canvas está en el DOM
        setTimeout(() => {
            renderizarGraficoBanco(canvasId, resultadosPorPlazo, montoInicial);
        }, 0); // setTimeout asegura que el DOM se haya actualizado antes de buscar el canvas
        
        console.log(`[GSB innerHTML] Finalizando para: ${banco.banco}`);
    }


    function renderizarGraficoBanco(canvasId, data, montoBase) { /* ... (sin cambios) ... */
        console.log(`[RGB] Renderizando gráfico para canvas ID: ${canvasId}`);
        ...
        try {
            ...
            console.log(`[RGB] Gráfico para ${canvasId} renderizado.`);
        } catch (e) {
            console.error(`[RGB] Error al crear Chart para ${canvasId}:`, e);
        }
    }

    function actualizarGraficoGeneralYResumen(resultados, montoBase) { /* ... (sin cambios) ... */
